...
namespace Edu.UI.Controllers
{
    ...
    public class ManageController : BaseController
    {
        ...
        {
            //0=>all,1=> is in
            UserLessonBLL userLessonBll=new UserLessonBLL();
            var list=await Task.Run(()=>userLessonBll.GetList(10, $"userid='{MyUserId}'", out int i, null, false, 1));
            return View(list);
        }
        ...
        public async Task<ActionResult> RemoveLogin(string loginProvider, string providerKey)
        {
            ...
            var result = await UserManager.RemoveLoginAsync(MyUserId, new UserLoginInfo(loginProvider, providerKey));
            if (result.Succeeded)
            {
        ...
        public async Task<ActionResult> AddPhoneNumber(AddPhoneNumberViewModel model)
        {
            ...
            // 生成令牌并发送该令牌
            var code = await UserManager.GenerateChangePhoneNumberTokenAsync(MyUserId, model.Number);
        ...
        {
            await UserManager.SetTwoFactorEnabledAsync(MyUserId, true);
            var user = await UserManager.FindByIdAsync(MyUserId);
            if (user != null)
            {
                await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
        ...
        {
            await UserManager.SetTwoFactorEnabledAsync(MyUserId, false);
            var user = await UserManager.FindByIdAsync(MyUserId);
            if (user != null)
            {
                await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
        ...
        {
            var code = await UserManager.GenerateChangePhoneNumberTokenAsync(MyUserId, phoneNumber);
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            var result = await UserManager.ChangePhoneNumberAsync(MyUserId, model.PhoneNumber, model.Code);
            if (result.Succeeded)
            {
                var user = await UserManager.FindByIdAsync(MyUserId);
                if (user != null)
        ...
        {
            var result = await UserManager.SetPhoneNumberAsync(MyUserId, null);
            if (!result.Succeeded)
            {
        ...
        public async Task<ActionResult> SetPassword(SetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                var result = await UserManager.AddPasswordAsync(MyUserId, model.NewPassword);
                if (result.Succeeded)
                {
                    var user = await UserManager.FindByIdAsync(MyUserId);
                    if (user != null)
        ...
        public async Task<ActionResult> ManageLogins(ManageMessageId? message)
        {
            ...
            var userLogins = await UserManager.GetLoginsAsync(MyUserId);
        ...
        public ActionResult LinkLogin(string provider)
        {
            ...
            return new AccountController.ChallengeResult(provider, Url.Action("LinkLoginCallback", "Manage"), MyUserId);
        }
        ...
        {
            var loginInfo = await AuthenticationManager.GetExternalLoginInfoAsync(XsrfKey, MyUserId);
            if (loginInfo == null)
            {
                return RedirectToAction("ManageLogins", new { Message = ManageMessageId.Error });
            }
            var result = await UserManager.AddLoginAsync(MyUserId, loginInfo.Login);
        ...
        {
            var user = UserManager.FindById(MyUserId);
            if (user != null)
            {
